...
def add_edge(edge_index, edge_index_pool, add_rate):
    num_edges = len(edge_index[0])
    num_adds = int(num_edges * add_rate)
    num_edge_index_pool = len(edge_index_pool[0])
    drop_rate_from_edge_index_pool = (
        num_edge_index_pool - num_adds) / num_edge_index_pool

    selected_edge_index = dropout_adj(
        edge_index_pool, p=drop_rate_from_edge_index_pool)[0]
    u = torch.cat((selected_edge_index[0], edge_index[0])).tolist()
    v = torch.cat((selected_edge_index[1], edge_index[1])).tolist()
